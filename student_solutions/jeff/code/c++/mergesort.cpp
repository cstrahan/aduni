...
void merge(int *array_a, int n_a, int *array_b, int n_b, int *array_c)
{
  ...
  while (n_a)       // while n_a is still positive
    {
      *array_c++ = *array_a++;      //places the value from array_a into array_c
      n_a--;                        //decrements n_a
    }
  while (n_b)       // while n_b is still positive
    {
  ...
  if (n > 1)
    {
      ...
      n_b = n - n_a;          // n_b gets the value of n_b - n
      sort (array_a, n_a);    // recurs using array_a and n_a
      sort (array_b, n_b);    // recurs using array_b and n_b
