...
/**
   Threading this with old and new tables to control table size.
   ...
   @version $Id: RoutingTable.java,v 1.4 2001/01/29 15:42:03 eklempne Exp $
*/
public class RoutingTable extends Thread
{
	...
	try
		...
		while(true)
			...
			sleep(30000); // tables refreshed every 30 seconds
		    }
	    }
    ...
    */
    ...
    /**
       ...
       if message == Pong: send ONLY to socketId (if -1, don't send);
       if message == Query: send to all sockets EXCEPT socketId;
       if message == QueryHit: send ONLY to socketId (if -1, don't send);
       if message == Push: send ONLY to socketId (if -1, don't send).
    */
	...
	Pong pong = new Pong(messageId, 0x01, 3, 3, 3, 3, ipAddress, 3, 3);
	pong.setSocketId(10);
	Vector v = new Vector();
	QueryHitObject qh = new QueryHitObject(messageId, 0x81, 3, 3, 3, 3, (short)3, ipAddress, 3, v, serventId);
	Push push = new Push(messageId, 0x40, 3, 3, 3, serventId, 3,
			     ipAddress, 3);

	RoutingTable rt = new RoutingTable();
	rt.start();

	System.out.println(RoutingTable.sendCheck(pong));
 	System.out.println(RoutingTable.
			   receiveCheck(messageId, 0x01, ipAddress, 1));
	System.out.println(RoutingTable.sendCheck(pong));
 	System.out.println(RoutingTable.
			   receiveCheck(messageId, 0x00, DUMMYPINGID, 2));
	System.out.println(RoutingTable.sendCheck(pong));

	System.out.println(RoutingTable.sendCheck(qh));
 	System.out.println(RoutingTable.
			   receiveCheck(messageId, 0x81, serventId, 3));
	System.out.println(RoutingTable.sendCheck(qh));
 	System.out.println(RoutingTable.
			   receiveCheck(messageId, 0x40, serventId, 4));
	System.out.println(RoutingTable.sendCheck(push));
