...
commit_line = '''<tr><td>Name:</td>
<td>%s</td><td>NDT District: %s</td>
</tr>
<tr><td>CEDA Region: %s</td>
<td>ADA Member: %s</td>
<td>Status: %s</td></tr>
...
validate_sql = 'SELECT school_id, school_name, ndt_district, ceda_region, ada_member FROM schools WHERE status=\'not reviewed\';'

invalidate_sql = 'SELECT school_id, school_name, ndt_district, ceda_region, ada_member FROM schools'

update_sql = 'SELECT date_registered,school_id,school_name,ndt_district,ceda_region,ada_member,status,school_password FROM schools'

update_changes_sql = 'UPDATE schools SET school_name = \'%s\',ndt_district = \'%s\',ceda_region=\'%s\',ada_member=\'%s\',status=\'%s\',school_password=\'%s\' WHERE school_id = \'%s\''
...
if (query_exists == 1):
	...
	if (query.Get('invalid')):
		...
		for num in range(query_size):
			if (query.Key(num)=='selected'):
				select_list = select_list + ',' + (query.Value(num))
		search = invalidate_sql + (where_sql % (select_list))
