...
commit_line = '''<tr><td>Title:</td>
<td>%s</td><td></td>
</tr>
<tr><td>Content Type: %s</td>
<td>Side: %s</td>
<td>Current Status: %s</td></tr>
<tr><td>Mod Total:</td><td>%s</td><td></td></tr>
...
validate_sql = 'SELECT c.list_id, c.title, t.team_name, p.first_names, p.last_name FROM content c, teams t, persons p WHERE c.team_id = t.team_id and c.posted_by = p.person_id and c.status = \'not reviewed\';'

invalidate_sql = 'SELECT c.list_id, c.title, t.team_name, p.first_names, p.last_name FROM content c, teams t, persons p WHERE c.team_id = t.team_id and c.posted_by = p.person_id'

#inv_where_sql = ' and p.person_id = (%s)'

update_sql = 'SELECT c.date_posted,c.list_id,p.first_names,p.last_name,s.school_name,c.team_id,c.title,c.content_type,c.side,c.status,c.message,c.mod_total,c.reference_id FROM content c, persons p, schools s, person_school_map psm WHERE c.posted_by = p.person_id and psm.person_id = p.person_id and s.school_id = psm.school_id'

update_changes_sql = 'UPDATE content SET title = \'%s\',content_type = \'%s\',side=\'%s\',status=\'%s\',mod_total=\'%s\' WHERE list_id = \'%s\''
