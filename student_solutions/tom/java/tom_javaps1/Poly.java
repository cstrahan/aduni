...

{
	...
	*/
    ...
    public int compareTo(Poly b)
    {/** Part of question 8, compareTo returns a 1, 0, -1 if polynomials are 
     *   greater than, equal to, or less than each other.
     */
	int ans = 0;
	int max = 0;
	
	if (this.degree() > b.degree()) ans = -1;
	else if (this.degree() < b.degree()) ans = 1;
	else for (int i = 0; i <= this.degree(); i++)
	    {
		if (this.coeffs[i] > b.coeffs[i]) ans = -1;
		else if (this.coeffs[i] < b.coeffs[i]) ans = 1;
		else ans = 0;
	    }
    }
