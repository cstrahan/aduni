...
### major errors addressed 4/26, tbh
...
<form method=get action="person_admin.py">
%s
%s
</form>
'''
...
update_line = '''<table>
<tr>Date Created:%s<br>ID #: %s Update this record <input type=checkbox name=selected value="%s" checked></tr>

<tr bgcolor='EEFFFA'><td>First Name:</td>
...
<tr bgcolor='white'><td>Last Name:</td>
...
<tr bgcolor='white'><td>Office/Mobile/Fax:</td>
...
commit_line = '''<tr bgcolor='EEFFFA'><td><b><font color='red'>First Name:</font></b></td>
<td>%s</td><td>Current Access:%s</td>
</tr>
<tr bgcolor='white'><td>Last Name:</td>
<td>%s</td>
<td>Current Status:%s</td></tr>
<tr bgcolor='EEFFFA'><td>Url:</td><td>%s</td><td>Email:%s</td></tr>
<tr bgcolor='white'><td>Street Address:</td><td>%s</td><td>School Liason for ID#: %s</td></tr>
<tr bgcolor='EEFFFA'><td>City:%s</td><td>State:%s</td><td>Zip:%s</td></tr>
<tr bgcolor='white'><td>Office Phone:%s</td><td>Mobile Phone:%s</td><td>Fax:%s</td></tr>
'''
...
update_psm_sql = '''
UPDATE person_school_map
SET website_contact = \'%s\'
WHERE person_id=%s and school_id=%s'''
...
school_contact_sql = '''SELECT psm.school_id,s.school_name from person_school_map psm, schools s WHERE psm.person_id=%s and psm.website_contact=true and psm.school_id = s.school_id'''

school_affiliation_sql = '''SELECT school_id,school_name from schools WHERE school_id IN (SELECT school_id from person_school_map WHERE person_id=%s)'''

liason_checkbox = '<input type=checkbox value=%s name=cancel_liason> Cancel this person as liason of %s'

liason_list = '<select name=set_liason><option value=0 selected>No school selected</option>%s</select>'

liason_cell = ''

liason_list2 = ''

liason_line = '<option value="%s">%s</option>'
...
if (query_exists == 1):
	if (query.Get('commit')):
		...
		for item in range(query_size):
			if (query.Key(item) == 'selected'):
				commit_sql = update_changes_sql % (query.Value(item+1),query.Value(item+2),query.Value(item+3),query.Value(item+4),query.Value(item+5),query.Value(item+6),query.Value(item+7),query.Value(item+9),query.Value(item+10),query.Value(item+11),query.Value(item+12),query.Value(item+13),query.Value(item+14),query.Value(item))
                                if (query.Key(item+8) == 'set_liason'):
                                        commit_psm_sql = update_psm_sql % ('t',query.Value(item),query.Value(item+8))
                                else:
                                        commit_psm_sql = update_psm_sql % ('f',query.Value(item),query.Value(item+8))
				try:
					db.DML(commit_sql)
                                except:
					error = error + 'There was a database problem updating the file of %s %s' % (query.Value(item+1),query.Value(item+3))
				try:
					db.DML(commit_psm_sql)
				except:
					pass
				commit_list = commit_list + commit_line % (query.Value(item+1),query.Value(item+2),query.Value(item+3),query.Value(item+4),query.Value(item+5),query.Value(item+6),query.Value(item+7),query.Value(item+8),query.Value(item+9),query.Value(item+10),query.Value(item+11),query.Value(item+12),query.Value(item+13),query.Value(item+14))		
	...
	if (query.Get('update')):
		...
		while db.GetRow(uresults) == Ns.OK:
                        liason_sql = school_contact_sql % (uresults[1])
                        sub = Ns.DbPoolGetHandle("subquery")
                        lresults = sub.Select(liason_sql)
                        if sub.GetRow(lresults) == Ns.OK:
                                liason_cell = liason_checkbox % (lresults.Get('school_id'),lresults.Get('school_name'))
                                del sub
                        else:
                                del sub
                                osearch = school_affiliation_sql % (uresults.Get('person_id'))
                                sub = Ns.DbPoolGetHandle("subquery")
                                oresults = sub.Select(osearch)
                                while sub.GetRow(oresults) == Ns.OK:
                                        liason_list2 = liason_list2 + liason_line % (oresults[0],oresults[1])
                                liason_cell = liason_list % (liason_list2)
                                del sub
                        update_list = update_list + update_line % (uresults.Get('date_registered'),uresults[1],uresults[1],uresults[2],uresults[3],uresults[4],uresults[5],uresults[6],uresults[7],uresults[8],liason_cell, uresults.Get('city'),uresults.Get('state'),uresults.Get('zipcode'),uresults.Get('office_phone'),uresults.Get('mobile_phone'),uresults.Get('fax'))
