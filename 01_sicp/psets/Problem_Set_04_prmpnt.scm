;;;; Primitive Painters
;;;; From MIT Press SICP web site.  Modifications were made to bring
;;;; the code in line with definitions in the second edition of the
;;;; textbook: e.g. xcor-vect instead of vector-xcor.
...
;;;; vectors: make-vect, xcor-vect, ycor-vect, scale-vect, add-vect, sub-vect
    ...
    (let ((coord-map
           (lambda (v)
             (add-vect (frame-origin frame)
                         (add-vect (scale-vect (xcor-vect v) (frame-edge1 frame))
                                     (scale-vect (ycor-vect v) (frame-edge2 frame)))))))
      (for-each
       (lambda (seg)
...
(define (fill-row-from-procedure row start-column end-column inv-map
  ...
  (let* ((inexr (exact->inexact row))
         ...
         (picture-start-x (xcor-vect picture-start-point))
         (picture-start-y (ycor-vect picture-start-point)))
    (if (> start-column end-column)
        'done
...
(define (fill-row-from-picture row start-column end-column inv-map
  ...
  (let* ((inexr (exact->inexact row))
         ...
         (picture-end-x (xcor-vect picture-end-point))
         (picture-end-y (ycor-vect picture-end-point))
...
(define (frame-mapping-info h v frame)
  (let* ((frame-org (frame-origin frame))
         ...
         (a11 (/ (xcor-vect (frame-edge1 frame)) h))
         (a12 (/ (xcor-vect (frame-edge2 frame)) v))
         (a21 (/ (ycor-vect (frame-edge1 frame)) h))
         (a22 (/ (ycor-vect (frame-edge2 frame)) v))
         (det (- (* a11 a22) (* a21 a12))))
    (if (<= (abs det) 1.e-4)
        (list 'degenerate)
        ...
        (let* ((b11 (/ a22 det))
               ...
               (inv-map
                (lambda (v)
                  (let ((v (sub-vect v frame-org)))
                    (let ((x (xcor-vect v)) (y (ycor-vect v)))
               ...
               (ycor-vects (map ycor-vect vertices))
               (lowest-row (max 0 (floor->exact (apply min ycor-vects))))
               (highest-row (min *last-screen-row* (ceiling->exact (apply max ycor-vects)))))
...
(define (make-inexact-vect vector)
  (make-vect (exact->inexact (xcor-vect vector))
             (exact->inexact (ycor-vect vector))))
